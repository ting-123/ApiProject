<template>
  ...
  >
    <div
      ref="invisible"
      v-show="!disableMove"
      class="vue-sticker__invisible"
      :style="invisible"
    ></div>
    <div
      ref="invisibleRotate"
      v-show="!disableRotate"
      class="vue-sticker__invisible"
    ></div>
    <div>
      <div v-show="showInner" class="vue-sticker__inner" :style="stylesInner">
        <div :style="backPicture">
          <div class="sticker__overflow" :style="innerImageOverflow">
            <div
              class="vue-sticker__inner"
              :class="`${className + prefix}backside-image`"
              :style="clip"
            ></div>
          </div>
        </div>
      ...
      <div :style="mainStyle">
        <div :style="stayWrapper">
          <div
            :class="`${className + prefix}main-image`"
            :style="mainPicture"
          ></div>
        </div>
      </div>
...
export default {
  ...
  created() {
    if (this.tipAnimation)
      startingAnimation = window.setInterval(this.showHelper, 3000);
  },
  mounted() {
    let movingEvents = ['mousemove', 'mouseleave', 'mousedown', 'mouseup'];
    let rotateEvents = ['mousemove', 'touchmove', 'touchend'];
    /**
     * Attach main handler to events of our invisible circle
     */
    movingEvents.map((eventName) => {
      this.$refs.invisible.addEventListener(eventName, (e) =>
    ...
    rotateEvents.map(
      (eventName) => {
        this.$refs.invisibleRotate.addEventListener(eventName, (e) =>
          this.calcRotate(e)
        );
      },
      { passive: true }
    ...
    this.$refs.invisibleRotate.addEventListener('mousedown', () => {
  ...
  methods: {
    calcRotate(e) {
      if (e.type === 'touchmove') {
        ...
        this.degree =
          (Math.atan2(e.offsetX - this.d / 2, e.offsetY - this.d / 2) * 180) /
          Math.PI;
      }
    ...
    movingHandler(e) {
      ...
      if (
        (!this.endPoint || e.offsetY < this.endPoint) &&
        e.type !== 'mouseleave'
      ) {
  ...
  computed: {
    ...
    mainStyle() {
      let height = !this.showInner
        ? this.d + 'px'
        : this.d - this.distance + 'px';
